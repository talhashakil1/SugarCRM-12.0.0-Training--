...
SourceTab.prototype.refreshData = function(first_load){
		...
		postData = 'index.php?module=Connectors&action=RetrieveSource&record={$RECORD}&to_pdf=true&source_id='+this.sourceId;
...
var SourceTabs = {
    ...
    setSearching : function(searching){
    	...
    	if(searching){
    		btn_search.value = "{$MOD.LBL_SEARCHING_BUTTON_LABEL}";;
    	}else{
    		btn_search.value = "      {$APP.LBL_SEARCH_BUTTON_LABEL}      ";
    	}
    },
    ...
    loadTab : function(tab, previousKey) {
     ...
     getSearchForm : function(source_id){
    	...
    	if(sTab.searchFormCache == ''){
			...
			postData = 'index.php?module=Connectors&action=GetSearchForm&merge_module={$module}&record={$RECORD}&source_id='+source_id;
			var cObj = YAHOO.util.Connect.asyncRequest('GET', postData, callback, null);
		} else{
